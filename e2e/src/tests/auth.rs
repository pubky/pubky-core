...
use crate::tests::utils::init_tracing;
...
async fn basic_authn() {
    ...
    client
        .signout_and_clear_session(&keypair.public_key())
        .await
        .unwrap();
...
async fn disabled_user() {
    ...
    client.put(&file_url, vec![]).await.unwrap();

    // Make sure the user can read their own file
    let response = client.get(&file_url).await.unwrap();
    assert_eq!(
        response.status,
        StatusCode::OK,
    ...
    let response = client.put(&file_url, vec![]).await.unwrap();
