...
impl ES256KeyPair {
    ...
    fn from_secret_key(secret_key: &[u8; 32]) -> Result<Self> {
        ...
        let secret = SecretKey::from_bytes(array)
    ...
    pub fn derive_from_main_secret_key(main_secret_key: &[u8; 32]) -> Result<Self> {
        let hkdf = hkdf::Hkdf::<sha2::Sha256>::from_prk(main_secret_key).expect("main secret key is large enough to be used as a PRK");
        let mut okm = [0u8; 32];
        hkdf.expand(b"jwt-signing-key", &mut okm).expect("32 is a valid output length");
    ...
    pub fn private_key_pem(&self) -> Result<String, p256::pkcs8::Error> {
        Ok(self.private_key.to_pkcs8_pem(Default::default())?.to_string())
    }

    /// Get the public key as a PEM string
    pub fn public_key_pem(&self) -> Result<String, p256::pkcs8::Error> {
        Ok(self.public_key().to_public_key_pem(Default::default())?.to_string())
    }
