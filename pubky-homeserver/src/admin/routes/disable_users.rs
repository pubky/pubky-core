use crate::{persistence::lmdb::tables::users::UserQueryError, shared::{HttpError, HttpResult, Z32Pubkey}};
use axum::{
    extract::{Path, State},
    http::StatusCode,
    ...
    if let Err(e)  = state.db.disable_user(&pubkey.0, &mut tx) {
        match e {
            UserQueryError::UserNotFound => {
                return Err(HttpError::new(StatusCode::NOT_FOUND, Some("User not found")))
            },
            UserQueryError::DatabaseError(_) => {
                return Err(HttpError::new(StatusCode::INTERNAL_SERVER_ERROR, Some("Database error")))
            }
        };
    }
    tx.commit()?;
...
mod tests {
    ...
    async fn test_disable_enable_tenant() {
        ...
        let response = server.post(format!("/tenants/{}/disable", pubkey).as_str()).await;
        assert_eq!(response.status_code(), StatusCode::OK);

        // Check that the tenant is disabled
        let user = db.get_user(&pubkey, &mut db.env.read_txn().unwrap()).unwrap();
        assert!(user.disabled);

        // Enable the tenant again
        let response = server.post(format!("/tenants/{}/enable", pubkey).as_str()).await;
        assert_eq!(response.status_code(), StatusCode::OK);
