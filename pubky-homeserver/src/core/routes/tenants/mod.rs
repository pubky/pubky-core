...
use crate::core::{
    layers::{
        ensure_host_matches_user_layer::EnsureHostMatchesUserLayer,
        ensure_path_capabilities_layer::EnsurePathCapabilitiesLayer,
    },
    sessions::SessionRequiredLayer,
    AppState,
};
...
pub fn router(state: AppState) -> Router<AppState> {
    ...
    let auth_routes = Router::new()
        .route(
            "/pub/{*path}",
            put(write::put)
                .layer(EnsureHostMatchesUserLayer::new())
                .layer(EnsurePathCapabilitiesLayer::new()),
        )
        .route(
            "/pub/{*path}",
            delete(write::delete)
