...
use crate::{
    ...
    }, persistence::sql::session::SessionRepository, shared::{HttpError, HttpResult}
};
...
pub async fn session(
    ...
    if let Some(secret) = session_secret_from_cookies(&cookies, pubky.public_key()) {
        if let Ok(session) = SessionRepository::get_by_secret(&secret, &mut (&mut state.sql_db.pool().into())).await {
...
pub async fn signout(
    ...
    if let Some(secret) = session_secret_from_cookies(&cookies, pubky.public_key()) {
        SessionRepository::delete(&secret, &mut (&mut state.sql_db.pool().into())).await?;
    }
