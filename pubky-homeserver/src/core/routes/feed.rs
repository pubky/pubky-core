...
///
/// ## Query Parameters
/// - `user` (**REQUIRED**): User public key to filter events for. Only events for this user will be streamed.
...
pub async fn feed_stream(
    ...
    let user_id = UserRepository::get_id(&user_pubkey, &mut state.sql_db.pool().into()).await?;
