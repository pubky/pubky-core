...
mod tests {
    ...
    async fn test_migrate() {
        ...
        lmdb_user1.created_at = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        lmdb_user1.used_bytes = 100;
        lmdb_user1.disabled = true;
        lmdb.tables.users.put(&mut wtxn, &user1_pubkey, &lmdb_user1).unwrap();

        // User2
        let user2_pubkey = Keypair::random().public_key();
        let mut lmdb_user2 = User::default();
        lmdb_user2.created_at = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        lmdb_user2.used_bytes = 200;
        lmdb_user2.disabled = false;
        lmdb.tables.users.put(&mut wtxn, &user2_pubkey, &lmdb_user2).unwrap();
        wtxn.commit().unwrap();
        ...
        assert_eq!(sql_user1.created_at.format("%Y-%m-%d %H:%M:%S").to_string(), DateTime::from_timestamp(lmdb_user1.created_at as i64, 0).unwrap().naive_utc().format("%Y-%m-%d %H:%M:%S").to_string());
        assert_eq!(sql_user1.disabled, lmdb_user1.disabled);
        assert_eq!(sql_user1.used_bytes, lmdb_user1.used_bytes);

        let sql_user2 = UserRepository::get(&user2_pubkey, &mut sql_db.pool().into()).await.unwrap();
        assert_eq!(sql_user2.created_at.format("%Y-%m-%d %H:%M:%S").to_string(), DateTime::from_timestamp(lmdb_user2.created_at as i64, 0).unwrap().naive_utc().format("%Y-%m-%d %H:%M:%S").to_string());
