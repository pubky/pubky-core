...
impl<R: oio::Write> oio::Write for WriterWrapper<R> {
    ...
    async fn close(&mut self) -> Result<opendal::Metadata> {
        ...
        if let Err(e) = self
            .entry_service
            .write_entry(&self.entry_path, &file_metadata).await
        {
            tracing::error!(
...
mod tests {
    use crate::{
        persistence::{files::opendal_test_operators::OpendalTestOperators, sql::event::{EventRepository, EventType}},
    ...
    async fn test_entry_layer() {
        for (_scheme, operator) in OpendalTestOperators::new().operators() {
            ...
            let events = EventRepository::get_by_cursor(0, 9999, &mut db.pool().into())
                .await
