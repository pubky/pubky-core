...
///
/// Typical flow:
/// 1) `AuthSignupFlow.start(...)` or `pubky.startAuthSignupFlow(...)`
/// 2) Show `authorizationUrl()` as QR/deeplink to the user’s signing device
/// 3) `awaitApproval()` to receive a ready `Session`
#[wasm_bindgen]
pub struct AuthSignupFlow(pub(crate) pubky::PubkySignupAuthFlow);

#[wasm_bindgen]
impl AuthSignupFlow {
    /// Start a flow (standalone).
    /// Prefer `pubky.startAuthSignupFlow()` to reuse a facade client.
    ///
    ...
    pub(crate) fn start_with_client(
        ...
        if let Some(ic) = invite_code {
            builder = builder.invite_code(ic);
        }

        Ok(AuthSignupFlow(builder.start()?))
    }
