...
use crate::{
    ...
    actors::{DEFAULT_HTTP_RELAY, http_relay_link_channel::EncryptedHttpRelayLinkChannel},
    cross_log,
...
/// **auth permission subscription** (long polling for a single auth token) you *hold on to*.
///
/// Use it like this:
/// 1. Construct with the builder
///    [`AuthPermissionSubscription::builder`] to override relay/client.
...
#[derive(Debug)]
pub struct AuthPermissionSubscription {
    client: PubkyHttpClient,
    rx: flume::Receiver<Result<AuthToken>>,
    abort: AbortHandle,
}

impl AuthPermissionSubscription {
    /// Create a builder for [`AuthPermissionSubscription`].
    pub fn builder(secret: [u8; 32]) -> AuthPermissionSubscriptionBuilder {
        AuthPermissionSubscriptionBuilder::new(secret)
    }
...
impl Drop for AuthPermissionSubscription {
    fn drop(&mut self) {
...
#[derive(Debug, Clone)]
pub struct AuthPermissionSubscriptionBuilder {
    relay_base_url: Url,
    secret: [u8; 32],
    client: Option<PubkyHttpClient>,
}

impl AuthPermissionSubscriptionBuilder {
    ...
    fn spawn_background_polling(encrypted_channel: EncryptedHttpRelayLinkChannel, client: PubkyHttpClient) -> Result<AuthPermissionSubscription> {
        ...
        let fut = async move {
            ...
            AuthPermissionSubscription::poll_for_token_loop(bg_client, encrypted_channel.clone(), tx)
                .await;
        };
    ...
    pub fn start(self) -> Result<AuthPermissionSubscription> {
...
mod tests {
    ...
    async fn subscribe_to_auth_token() {
        ...
        let subscriber = AuthPermissionSubscription::builder(encrypted_channel.secret().to_owned())
